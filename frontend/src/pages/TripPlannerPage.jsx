...
export default function TripPlannerPage({ tourist, geofences = [], onSimulateDeviation }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      {/* Header */}
      <div className="bg-[#FFD8BD]/85 backdrop-blur-md border border-slate-200/80 rounded-2xl p-6 shadow-md flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-slate-900">Tourist Trip Itinerary & Route Deviation Monitor</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white/90 shawdow-xl backdrop-blur-md border border-slate-200/80 rounded-2xl p-5 shadow-md space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                <span className="text-[10px] text-slate-500 uppercase font-bold block">Starting Point</span>
                <span className="font-bold text-slate-900">{trip.startingLocation}</span>
              </div>

              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                <span className="text-[10px] text-slate-500 uppercase font-bold block">Destination</span>
                <span className="font-bold text-emerald-700">{trip.destination}</span>
              </div>

              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                <span className="text-[10px] text-slate-500 uppercase font-bold block">Registered Travel Dates</span>
                <span className="font-semibold text-slate-800">{trip.startDate} to {trip.endDate}</span>
              </div>

              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                <span className="text-[10px] text-slate-500 uppercase font-bold block">Planned Waypoint Corridor</span>
                <span className="font-mono text-slate-700 text-[11px] block">{trip.plannedRoute}</span>
              </div>
            </div>

            {/* Deviation Simulation Button */}
            <div className="pt-2 border-t border-slate-200 space-y-2">
              <span className="text-[10px] text-slate-500 uppercase font-bold block">SIH Evaluator Route Anomaly Test</span>
              <button
                disabled={loading}
