...
export default function CinematicHeroMap({ onScenarioTrigger }) {
  ...
  return (
    <div className="relative w-full h-[92vh] min-h-[650px] overflow-hidden rounded-3xl border-2 border-emerald-500/30 shadow-2xl bg-slate-100">
        ...
        >
          <Popup>
            <div className="p-2 space-y-1 text-xs text-slate-800">
              ...
              <p className="font-semibold text-amber-700">Risk Score: {riskScore}/100 ({riskLevel})</p>
            </div>
          </Popup>
        </Marker>
          ...
          <Marker position={POLICE_STATION_LOC} icon={createCinematicIcon('#3B82F6', true, '🚓')}>
            <Popup>
              <div className="p-2 text-xs">
                ...
                <p className="text-slate-700 font-semibold mt-1">Status: En Route to SOS Lock</p>
              </div>
            </Popup>
          </Marker>
        )}
      ...
      <div className="absolute top-4 left-4 right-4 z-10 flex flex-wrap items-center justify-between gap-3 pointer-events-none">
        ...
        {/* Hero Branding Badge */}
        <div className="pointer-events-auto bg-white/90 backdrop-blur-xl border border-slate-200/80 px-4 py-2 rounded-2xl shadow-xl flex items-center space-x-3">
          <div className="w-3 h-3 rounded-full bg-emerald-500 animate-ping" />
          <div>
            <h2 className="text-sm font-black text-slate-900 tracking-wide">SAFE-TOUR CINEMATIC COMMAND ENGINE</h2>
        ...
        <div className="pointer-events-auto bg-white/90 backdrop-blur-xl border border-slate-200/80 px-3.5 py-2 rounded-2xl shadow-md flex items-center space-x-3">
          ...
          <button
            ...
            className="px-2.5 py-1 text-[11px] font-bold rounded-xl bg-slate-100 hover:bg-slate-200 text-emerald-800 border border-slate-300 transition-colors"
          >
          ...
          <button
            ...
            className={`px-2.5 py-1 text-[11px] font-bold rounded-xl transition-colors ${
              followMe ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700 border border-slate-300'
            }`}
          >
        ...
        <div className="absolute top-20 left-4 z-10 max-w-md w-full pointer-events-auto transition-all animate-bounce-short">
          <div className={`p-4 rounded-2xl backdrop-blur-xl border-2 shadow-2xl space-y-2 ${
            activeAlert.type === 'WARNING'
              ? 'bg-yellow-50/95 border-yellow-300 text-yellow-900'
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono font-bold bg-slate-200/80 px-2 py-0.5 rounded text-slate-800">
      ...
      <div className="absolute top-20 right-4 z-10 pointer-events-auto bg-white/90 backdrop-blur-xl border border-slate-200/80 p-4 rounded-2xl shadow-xl space-y-2 w-48">
        <div className="flex items-center justify-between text-xs">
          <span className="font-extrabold text-slate-700 uppercase tracking-wider">AI Risk Meter</span>
          <span className={`font-black px-2 py-0.5 rounded text-[10px] ${
            riskLevel === 'CRITICAL' ? 'bg-red-100 text-red-800 border border-red-300' :
            riskLevel === 'HIGH' ? 'bg-orange-100 text-orange-800 border border-orange-300' :
        ...
        <div className="relative pt-1">
          ...
          <div className="overflow-hidden h-2.5 text-xs flex rounded-full bg-slate-200 border border-slate-300">
            <div
        ...
        {responseEta && (
          <div className="pt-2 border-t border-slate-200 text-[11px] text-blue-800 font-bold flex items-center justify-between">
            <span>Response ETA:</span>
            <span className="bg-blue-100 px-2 py-0.5 rounded text-blue-800 border border-blue-300">{responseEta}</span>
          </div>
        )}
      </div>

      {/* Bottom Floating Control Bar: 13-Scene Judge Demo Player */}
      <div className="absolute bottom-4 left-4 right-4 z-10 pointer-events-auto bg-white/90 backdrop-blur-2xl border-2 border-emerald-500/40 p-3 sm:p-4 rounded-3xl shadow-2xl space-y-3">
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-xl text-[10px] font-bold transition-all whitespace-nowrap ${
                  isActive
                    ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-lg scale-105 font-black'
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-2.5">
          
          <div className="flex items-center space-x-2">
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs rounded-xl shadow-lg transition-all flex items-center space-x-1.5"
            >
            ...
            <button
              onClick={handleNext}
              className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs rounded-xl border border-slate-300 transition-colors flex items-center space-x-1"
            >
            ...
            <button
              ...
              className="p-2 bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 rounded-xl border border-slate-300 transition-colors"
              title="Restart Demo"
            >
          ...
          <div className="text-[11px] text-slate-600 font-semibold hidden lg:block">
